...
def plot_ent_L_flow(_model, _file1, _file2=None, _file3=None, _file4=None, _file5=None, _file6=None,
    ...
    ax0.plot(L, [s/l for s, l in zip(S_page, L)], 'x-', label="Page", c='k')
    ax0.plot(L, [s/l for s, l in zip(S, L)], '.-', label="$J_z=0$", c='C0')
    if _file2 is not None:
        with open(os.path.join(proj_root, 'data/ent_mid_L_flow', _model, _file2), 'r') as csvfile:
            ...
            for i, row in enumerate(plots):
                L.append(float(row[0]))
                S.append(float(row[1]))
        ax0.plot(L, [s/l for s, l in zip(S, L)], '.-', label="$J_z=0.01$", c='C1')
    if _file3 is not None:
        with open(os.path.join(proj_root, 'data/ent_mid_L_flow', _model, _file3), 'r') as csvfile:
            ...
            for i, row in enumerate(plots):
                L.append(float(row[0]))
                S.append(float(row[1]))
        ax0.plot(L, [s/l for s, l in zip(S, L)], '.-', label="$J_z=0.1$", c='C2')
...
if __name__ == "__main__":
    ...
    model = 'heisenberg'
    file1 = 'ent_mid_L_flow_heisenberg_L_10_16_4_Nup_5_8_4_pauli_0_obc_dis_1000_J_1_1_0_W_5.dat'
    file2 = 'ent_mid_L_flow_heisenberg_L_10_16_4_Nup_5_8_4_pauli_0_obc_dis_1000_J_1_1_0.01_W_5.dat'
    file3 = 'ent_mid_L_flow_heisenberg_L_10_16_4_Nup_5_8_4_pauli_0_obc_dis_1000_J_1_1_0.1_W_5.dat'
    ...
    plot_ent_L_flow(model, file1, file2, file3, file4, _save=False)
