...
def plot_ener_spec(_model, _file1, _file2=None, _save=False):
    ...
    with open(os.path.join(proj_root, 'data/ener', _model, _file1), 'r') as csvfile:
    ...
    if _file2 is None:
        ...
        ax0.set_title(_file1.replace('ener_', '').replace('_', '\_').replace('.dat', ''))
        ...
        with open(os.path.join(proj_root, 'data/ener', _model, _file2), 'r') as csvfile:
            plots = csv.reader(csvfile, delimiter='\t')
            E_2 = []
            for i, row in enumerate(plots):
        ...
        ax1.set_title(_file2.replace('ener_', '').replace('_', '\_').replace('.dat', ''))
...
if __name__ == "__main__":
    ...
    model = 'heisenberg'
    file1 = 'ener_heisenberg_L_8_obc_J_1_1_1_W_0.5.dat'
    file2 = 'ener_heisenberg_L_8_obc_J_1_1_1_W_8.dat'

    plot_ener_spec(model, file1, file2, _save=False)
