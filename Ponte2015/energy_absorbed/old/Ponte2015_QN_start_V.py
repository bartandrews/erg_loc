...
def drive(t):
    ...
    if 0 < t < T_1:
        #print(f"input={t}, output=-1")
        return -1  # yields V
    elif T_1 < t < T:
        #print(f"input={t}, output=+1")
        return +1  # yields H_0
    else:
        #print(f"input={t}, return 0")
...
def realization(itr):
    ...
    # E_Tinf = H_init.trace(time=0) / (2**L)
    # E, phi = H_init.eigh(time=0)
    E_Tinf = H.trace(time=-np.finfo(float).eps) / (2 ** L)
    E, phi = H.eigh(time=-np.finfo(float).eps)
